...
function ProfileAvatar({
  ...
  const size = 110;
  const className = 'rounded-full object-fill';

  const baseAvatar = data?.base?.user_avatar;
  const web3BioAvatar = data?.web3Bio?.find(v => v.avatar)?.avatar ?? '';
  const avatarAlt = data?.base?.user_nick_name;
  const showBaseAvatar = baseAvatar || !web3BioAvatar;

  return (
    <div className="-mt-[104px] md:mt-0 relative">
      {showBaseAvatar && (
        <Avatar
          size={size}
          src={baseAvatar}
      ...
      {!showBaseAvatar && (
        <>
          <Image src={web3BioAvatar} alt={avatarAlt} className={className} width={size} height={size} unoptimized />
          <Image
            src={Web3BioIcon}
            alt="web3bio"
