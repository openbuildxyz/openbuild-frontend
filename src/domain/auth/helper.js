...
function gotoOauthUrl(type, from) {
  ...
  case 'google':
    ...
    break;
  case 'github':
    url = `https://github.com/login/oauth/authorize?client_id=${process.env.NEXT_PUBLIC_GITHUB_ID}&scope=user:email&redirect_uri=${redirectUri}?from=${encodeURIComponent(from)}`;
    break;
  case 'aspecta':
